...
This skill is the distilled rulebook for writing Cohere docs in this repo. Read it before starting a draft. Apply the per-page-type template to your draft. Run the verification checks before review.
...
Eighteen principles govern Cohere docs pages, distilled from the AI Transport wireframe workings (April 2026). Three groups: general (apply to every page), page-specific (depend on page type), and mechanical (binary checks).
...
8. **Feature pages lead with the mechanism.** The first heading is "How it works", not "the problem with X". Lead with how Cohere solves the job; problem framing is a lightweight aside or lives on the concept page.
...
12. **Framework pages: intentional design framing, not "missing features".** Frame the framework's scope as a deliberate boundary the framework chose, and Cohere as the layer that fills the gap. Never "the framework can't do X" — always "the framework intentionally doesn't do X". Sibling framework pages (for example UI vs Core) use verbatim-aligned capability bullets.
...
5. **Implementation.** Server code, client code, verification-in-prose ("open the channel in the Cohere dashboard to see…"), durable-execution guidance inline where applicable.
...
3. **Model.** How the concept maps to Cohere's primitives. Diagrams welcome.
...
3. **Pivot.** A clearly-marked "How Cohere solves this" or equivalent capability section. Everything below the pivot is positive-forward.
4. **Comparison table** (optional). Direct HTTP vs durable sessions, default cache vs Cohere, and so on. Each row is checkable.
5. **How Cohere implements this.** The primitives that back the model — link out to concepts.
...
5. **Server-side code.** The Cohere-specific code, clean. No 40-line auth boilerplate.
6. **Client-side code.** The Cohere-specific code, clean.
...
4. Where the SDK exposes both a core and a framework-bound entry point (for example `@cohere/ai-transport` and `@cohere/ai-transport/vercel`), pick the entry point your snippet's audience uses and follow its option shape.
