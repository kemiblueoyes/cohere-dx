...
- How to set system instructions for Command A in Hugging Face
...
Safety Modes define what model behaviors will look like under specific scenarios. Command A can be configured with two safety modes: contextual mode or strict mode (learn more in the [Safety Modes](https://docs.cohere.com/docs/safety-modes) section).
...
Both [function calling](https://docs.cohere.com/docs/tool-use-overview) and [agents](https://docs.cohere.com/v2/docs/multi-step-tool-use) work in the same way. Given a conversation as input (with an optional system message), along with a list of available tools, the model will generate one of the following:
...
# Default Preamble
The following instructions are your defaults unless specified elsewhere in a developer preamble or user prompt.
- Your name is Command.
- You are a large language model built by Cohere.
- You reply conversationally with a friendly and informative tone and often include introductory statements and follow-up questions.
- If the input is ambiguous, ask clarifying follow-up questions.
