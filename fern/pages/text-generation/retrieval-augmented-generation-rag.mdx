...
description: Generate text with external data and inline citations using Retrieval-augmented generation (RAG) and Cohere's Chat API.
...
---
Retrieval Augmented Generation (RAG) is a method for generating text using additional information fetched from an external data source, which can greatly increase the accuracy of the response. For a conceptual introduction to RAG, see [Introduction to RAG on LLM University](https://cohere.com/llmu/introduction-to-rag). 

When used with [Command A+](/docs/command-a-plus), [Command A](/docs/command-a), or [Command R7B](/docs/command-r7b), the [Chat API](/reference/chat) makes it easy to generate text that is grounded by supplementary information.

The code example below produces a grounded answer to `"Where do the tallest penguins live?"`, and inline citations based on the provided documents.
...
The RAG workflow has three steps:
...
RAG doesn't guarantee accuracy. It gives a model context which informs its replies. But if the provided documents are themselves out-of-date, inaccurate, or biased, whatever the model generates will be too. Also, RAG doesn't guarantee that a model won't hallucinate. It reduces the risk, but doesn't necessarily eliminate it. That's why it's important to include inline citations, allowing users to verify information retrieved.
