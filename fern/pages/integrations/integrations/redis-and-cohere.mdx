...
- You have Redis installed (refer to the [Redis documentation](https://www.redisvl.com/overview/installation.html#redis-stack-local-development) for instructions).
- You have a Cohere API Key (available from the [dashboard](https://dashboard.cohere.com/api-keys)).
...
To configure a Redis index you can either specify a `yaml` file or import a dictionary. In this tutorial we will be using a `yaml` file with the following schema. Either use the [`yaml` file in this repo](https://github.com/cohere-ai/cohere-developer-experience/blob/main/notebooks/configs/redis_guide_schema.yaml), or create a `.yaml` file locally with the following configuration.
...
Refer to [Cohere’s Embed Models](/docs/embed-2) for details around the available embedding models from Cohere and their respective dimensions.
